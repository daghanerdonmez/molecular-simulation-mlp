...
def generate_pipe_tree_yaml(
    ...
    """
    Generates a random tree of pipes with ring type receivers, a single emitter, and sinks.
    ...
    # -------------------------------
    # 4) Place Ring Receivers with countingType 1
    # -------------------------------
    # We'll place ring receivers with countingType 1 along all pipes at regular intervals
    ...
    for pipe_idx in range(pipe_count):
        ...
        # Define ring thickness (consistent for all rings)
        thickness = receiver_thickness
        
        # Calculate how many rings we can fit along the pipe (1 thickness apart)
        # Start from z=0 and place rings at regular intervals
        z_position = 0.0
        while z_position < pipe_length-thickness:
            receiver_data = {
            ...
            # Move to next position (1 thickness apart)
    ...
    for pipe in pipes:
        if len(pipe['right_connections']) == 0:
            ...
            # Add a ring receiver with countingType 0 at the end of each leaf pipe
            # Account for the thickness of the receiver
            thickness = receiver_thickness
            receiver_data = {
                'type': 'Ring type with thickness',
                'name': f"#{receiver_id}-Ring type with thickness",
...
def generate_n_times(n):
    for i in range(n):
        ...
        # Save to file
        with open(f"configs_ring/network_config_{i}.yaml", "w") as f:
            f.write(yaml_output)
